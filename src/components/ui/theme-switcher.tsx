"use client";
/* B03: the supplied component used state, effects and localStorage with no
   "use client" directive — it breaks as a server component in the App Router.
   B04: it also read localStorage during render, so the server emitted one theme
   and the client another. Initial state is now deterministic; storage is read
   in an effect. §5: dark is the default, `system` is respected. */
...
export const ThemeSwitcher = () => {
  ...
  const opts: { value: TTheme; label: string; d: string }[] = [
    {
      value: "system",
      label: "system",
  ...
  return (
    <fieldset className="flex h-8 p-0 m-0 rounded-[999999px] border border-shadow">
      ...
      {opts.map((o, i) => (
        <div key={o.value} className={i === 0 ? "-mt-px -ml-px" : i === 2 ? "-mt-px -mr-px" : "-mt-px"}>
          <input
            aria-label={o.label}
            id={`theme-switch-${o.value}`}
            type="radio"
            value={o.value}
            ...
            <svg height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16">
              <path
                ...
                className={`group-hover:fill-gray-1000 duration-150 ${
                  theme === o.value ? "fill-gray-1000" : "fill-gray-700"
                }`}
              />
            </svg>
          </label>
